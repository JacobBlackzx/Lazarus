...
type
  ...
  Tfrmcad_planoconta = class(Tfrmcad_padrao)
    ...
    procedure dbgTelaDblClick(Sender: TObject);
...
procedure Tfrmcad_planoconta.dbgTelaDblClick(Sender: TObject);
begin
//
  if plano.localiza(qrPesquisaid_plano.Value) then
     begin
       pgc.PageIndex        := 1;
       edtCodigo.Text       := IntToStr(plano.id_plano);
       edtDescricao.Text    := plano.descricao;
       cboTipo.Text         := plano.tipo;
     end;
end;
